...
instance_array=($(nova list --all-tenants --fields ${fields} |\
    awk -W posix -F'|' '$2 ~ /[[:alnum:]-]{36}/{
      ...
      secgrp=match($8,/default/);gsub(" ","",$2);gsub(" ","",$3);
...
for instance_data in ${instance_array[@]}; do
    ...
    if [[ $packet_loss -ge ${CRITICAL_PACKET_LOSS} ]]; then
        ...
        alert_array["critical"]+="{${hypervisor}::${name}(${uuid})::${public_ip}::${packet_loss}%} "
        ...
        alert_array["warning"]+="{${hypervisor}::${name}(${uuid})::${public_ip}::${packet_loss}%} "
    else
        print_val "OK: {${hypervisor}::${name}(${uuid})::${public_ip}::${packet_loss}%}"
        alert_array["ok"]+="{${hypervisor}::${name}(${uuid})::${public_ip}::${packet_loss}%} "
    fi
done
