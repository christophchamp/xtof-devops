...
# NOTE: hacked up to run okay on python 2.6.6 by brian.vanstaalduine@gettyimages
...
DEFAULT_SCHEME = '{0}.nova.hypervisors'.format(socket.gethostname())
...
def output_metric(name, value):
    print '{0}\t{1}\t{2}'.format(name, value, int(time.time()))
    ...
    for hv in hypervisors:
        for key, value in hv.to_dict().iteritems():
            if key in METRIC_KEYS:
                output_metric('{0}.{1}.{2}'.format(args.scheme, hv.hypervisor_hostname, key), value)
