...
ALERT_NAME="CheckCinderServicesState"
VERSION="Version 1.0"
...
if [[ ${#alert_array["critical"]} -gt 0 ]]; then
    echo "${ALERT_NAME} CRITICAL: The following Cinder services are down: {${alert_array["critical"]}}"
    exit $STATE_CRITICAL
else
    #echo "${ALERT_NAME} OK: All Cinder services are up: {${alert_array["ok"]}}"
